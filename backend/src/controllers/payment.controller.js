...
import { kv } from '@vercel/kv';  // Import the appropriate Redis client library
...
const getOrderSession = asyncHandler (async (req, res, next) => {
    try {
...
const addShippingInfo = asyncHandler( async (req, res, next) => {
    try {
...
const getStripePublicKey = asyncHandler(async (req, res, next) => {
    try {
        if (req.orderUser) {
            const info = {
                status: true,
...
export {createOrderSession,getOrderSession,addShippingInfo,getShippingInfo,confirmOrder,processPayment,getStripePublicKey}